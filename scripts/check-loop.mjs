...
assert(!/WAVE LIVE/.test(uiSrc), 'no green WAVE LIVE copy');
assert(/--amber/.test(css.split('.start-wave')[1].split('.dock-tools')[0]), 'START WAVE stays amber');
assert(!/#22c55e|#16a34a|#4ade80|limegreen/.test(css.split('.start-wave')[1].split('.dock-tools')[0]), 'START WAVE is not green');
assert(/Port — Last Line/.test(html) && /PORT/.test(html) && /LAST LINE/.test(html), 'EN title Port — Last Line');
assert(/Start Patrol/.test(html) && !/>Deploy/.test(html), 'HQ CTA Start Patrol');
...
const drawSrc = await readFile(new URL('../draw.mjs', import.meta.url), 'utf8');
assert(/drawFog|lighthouse/.test(drawSrc), 'flat overlay fog + lighthouse');
assert(/level >= 2/.test(drawSrc), 'tower L2/L3 extra detail');
...
const scoutSvg = await readFile(new URL('../assets/ships/scout-l1.svg', import.meta.url), 'utf8');
const swarmSvg = await readFile(new URL('../assets/ships/swarm-l1.svg', import.meta.url), 'utf8');
assert(scoutSvg.includes('#7fd4ff') && !scoutSvg.includes('#b8ffd9'), 'scout asset ring cyan not mint');
assert(swarmSvg.includes('#b8ffd9') && !swarmSvg.includes('#7fd4ff'), 'swarm asset ring mint not scout cyan');
assert((await readFile(new URL('../assets/towers/cannon-l3.svg', import.meta.url), 'utf8')).length > 40, 'cannon L3 sprite');
assert((await readFile(new URL('../assets/towers/tesla-l2.svg', import.meta.url), 'utf8')).includes('#7fd4ff'), 'tesla sprite');
const mapA = await readFile(new URL('../assets/maps/a.png', import.meta.url));
assert(mapA[0] === 0x89 && mapA[1] === 0x50 && mapA.length > 1000, 'map A PNG base');
